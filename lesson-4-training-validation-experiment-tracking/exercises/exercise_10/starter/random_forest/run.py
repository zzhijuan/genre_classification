...
def get_inference_pipeline(args):
    ...
    ############# YOUR CODE HERE
    numeric_transformer = make_pipeline(SimpleImputer(strategy="mean"), StandardScaler())# USE make_pipeline to create a pipeline containing a SimpleImputer using strategy=median
    ...
    ############# YOUR CODE HERE
    nlp_transformer = make_pipeline(SimpleImputer{strategy="constant", fill_value=""), reshape_to_1d, TfidVectorizer(binary=True)# USE make_pipeline to create a pipeline containing a SimpleImputer with strategy=constant and
    ...
    pipe = Pipeline(steps = ["preprocessor", preprocessor, 
                             "classifier", RandomForestClassifier(**model_config)])# CREATE a Pipeline instances with 2 steps: one step called "preprocessor" using the
